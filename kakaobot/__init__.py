...
class Client:
	...
	error_text : String, Optional (default : "Error occured")
		the string that will be talked by bot when someone say that is not applyed.
	"""
	def __init__(self, port=5000, kboard = Kboard(), error_text = "Error occured"):
		self.kboard = kboard
		self._command = {}
			...
			while(True):
				if get_data.type == "text":
					if get_data.content in self._command.keys():
						target = self._command[get_data.content]()
						break
					else:
						if self._prefix_command:
							temp_prefix = get_data.content.split(" ")[0]
							if temp_prefix in self._prefix_command.keys():
								target = self._prefix_command[temp_prefix](get_data.content[len(temp_prefix)+1:])
								break
						if self._regex_command:
							...
							for i in self._regex_command.keys():
								...
								if temp_regex:
									swt = True
									target = self._regex_command[i](temp_regex.groups())
									break
							if swt: break
						if self._extra:
							target = self._extra(get_data.content)
							break
						target = Message(text = self.error_text)
						break
				else:
					target = Message(text = self.error_text)
					break
